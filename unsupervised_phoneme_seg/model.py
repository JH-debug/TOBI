...
from dataloader import phoneme_seg_dataset
...
def main(config: DictConfig):
    data, _, _ = phoneme_seg_dataset.get_dataset(config)
    spect, seg, phonemes, length, wav_file = data[0]
    spect = spect.unsqueeze(0)

    model = NextFrameClassifier(config)
    out = model(spect)
    print(out)
    print(model.pred_steps)
